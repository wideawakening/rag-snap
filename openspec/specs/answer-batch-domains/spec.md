# answer-batch-domains Specification

## Purpose

Route the questions of a batch manifest to operator-declared requirement domains, so each question is
answered with the context and retrieval terms of the domain it belongs to. Routing is manifest data
only — an optional top-level `domains` list resolved in code against each question's `id` (falling
back to its `source`), supplying per-question context to the model turn and keywords to retrieval, and
recording the applied domain on the result. A manifest without the list behaves exactly as it did
before this capability existed. This covers the `rag-cli.rag answer batch` pipeline shared by the
CLI's direct run path and the daemon.
